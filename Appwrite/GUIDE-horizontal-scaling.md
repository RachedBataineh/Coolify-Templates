...
> **Which compose file**: for fresh multi-node deployments use `docker-compose-appwrite-official-external-dbs.yaml` — the official Coolify template minus the bundled databases, with the queue Redis AUTH fix embedded and 9 required connection variables (`_APP_DOMAIN`, `_APP_DB_HOST/USER/PASS`, `_APP_REDIS_HOST`, `_APP_CLICKHOUSE_HOST/USER/DB`, `_APP_USAGE_PASS`). Its timer services (scheduler, maintenance, interval) carry a single `timers` profile, so where this guide says `COMPOSE_PROFILES=combined,timers` on node A and `combined` on nodes B+, with that file you set `COMPOSE_PROFILES=timers` and `COMPOSE_PROFILES=` (empty) respectively. The older `docker-compose-appwrite-external-dbs.yaml` (custom topology with combined/separate worker profiles) still works and follows the guide's vocabulary verbatim.
...
On node B's Coolify (or the same Coolify, different server destination): New Resource → Docker Compose → paste the **same** file you deployed on node A (`docker-compose-appwrite-official-external-dbs.yaml` recommended).

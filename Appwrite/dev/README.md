...
| Usage tab stays empty; scheduler logs `usage_schema_not_ready` forever; API boot log shows ClickHouse `403` | The DSN built from the template's ClickHouse vars doesn't match your server — wrong user or database name (the bundled container uses `appwrite`/`appwrite`; a Coolify standalone ClickHouse uses `default`/`default`) | The template makes `_APP_CLICKHOUSE_USER`/`_APP_CLICKHOUSE_DB` required (red border) — set them to your server's values and redeploy; the DSNs rebuild automatically. On stacks deployed before these existed, set `_APP_CONNECTIONS_DB_USAGE`/`_DB_EXECUTIONS` explicitly to `http://<user>:<pass>@<host>:8123/<db>`. Verify: `curl "http://<user>:<pass>@<host>:8123/?query=SELECT%201"` returns `1`, and after boot the database contains `projects_usage_*` tables |
